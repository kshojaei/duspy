...
import miepython
'''############# Input Values ############## '''
ne_guess = 1.55e15                # Electron Density [1/m3] (Initial Guess)
Vp_guess = -1.8905896692302606# Particle Potential (eV) (Initial Guess)
ni_guess = 2.1679219616341384e+16 # Ion Density [1/m3]
...
EN_guess = 637.0          # Reduced Electric Field [Td]
p = 8                           # Pressure [Pa]
...
bb_guess = 0.43E-13               # Amplitude of detachment cross section
...
def escape_factor_1p1(pressure):
    slope = (1.4E-3 - 2.5E-3)/(200 - 30)
    rel = 1.4E-3 + slope*(pressure - 200)
    return rel/5
# Escape Factor (1P1) Ex Argon lmda_2: 104.8E-9 nm
def escape_factor_3p1(pressure):
    slope = (1E-3- 2.0E-3)/(200 - 30)
    rel = 1E-3 + slope*(pressure - 200)
    return rel/5
# pascal to mTorr conversion
press = p * (1000/133.322)
...
g_escape_1 = escape_factor_3p1(press)              # Escape Factor (3P1) Ex Argon lmda_1: 106.7E-9 nm
g_escape_2 = escape_factor_1p1(press)              # Escape Factor (1P1) Ex Argon lmda_2: 104.8E-9 nm
...
def detach(Vp, x, prop_factor, e_temperature, n_e, n_i):
    v = n_e / n_i
    t = (e_temperature * 11604.52500617) / Ti
    nu = (co.m_e / mass_one)
    charge = (4 * np.pi * co.epsilon_0 * rp * Vp) / co.e
    be = np.power(co.e,2) /(4*np.pi*co.epsilon_0*rp*co.k*e_temperature*11604.52500617)
    sigma2q  = ((1/be) *(1 - t*be*charge)) / (t + 0.899*v*np.sqrt(t/nu)*sp.special.erfc(-0.494*be*charge))
    sigma = (np.sqrt(sigma2q) *co.e)/ (4 * np.pi * co.epsilon_0 * rp)
    hump_eng = (-E_fermi) + Vp
...
def EERL(E, species_one, species_two, f, e_temperature, e_density, i_density, floating_potential):
    ...
    d_p_in = npp * (np.multiply((2 * co.m_e / m_particle), elas(floating_potential,
                                lamd_f,E)) + attach_dist(floating_potential, E, 
                                                         e_temperature, e_density, i_density))
        ...
        with open(CS_dir) as fp:
            ...
            for i in processes:
                while (i['target']) == self.species_name and i['kind'] == 'EXCITATION':
                    ...
                    print("Ar Excitation Frequency (1/s)",vexc )
        ...
        for Vpp in np.linspace(4, -15):
            for nee in np.linspace(1E12, 1E18):
                ...
                titan.add_process(kind = "ATTACHMENT", target="Particle", 
                                  data= np.c_[E, attach_dist(Vp,E, self.electron_temperature,
                                                             ne, self.ion_density)])
                titan.add_process(kind= "ATTACHMENT", target="Particle", 
                                  data= np.c_[E, detach(Vp,E,self.bb_g, 
                                                        self.electron_temperature,ne, self.ion_density)])
                ###############################################################################
                ...
                freq_detach = np.sqrt((2 * co.e)/co.m_e) * x * E * z1 * detach(Vp, E, self.bb_g, 
                                                                               self.electron_temperature,
                                                                               ne, self.ion_density)
                freq_attach = np.sqrt((2 * co.e)/co.m_e) * x * E * z1 * attach_dist(Vp, E,
                                                                                    self.electron_temperature,
                                                                                    ne, self.ion_density)
                ...
                # Sum of Ion frequencies for the potential distribution
                def ion_freq_dist(mean,x, e_temperature, n_e, n_i):
                    res = []
                    sa = 4 * np.pi * (rp ** 2)
                    a_coeff = sa * n_i * (((co.k*Ti)/(2* np.pi * mass_one))** 0.5)
                    vp = vp_dist(mean, x, e_temperature, n_e, n_i)
                    ...
                    vp_range = np.linspace(vp[0] - vp[1], vp[0] + vp[1],num)
                    for k in vp_range:
                        res.append(a_coeff * (1-((co.e * k)/(co.k * Ti))))
                    return np.sum(res)
                ...
                def equations(p):
                    ...
                    eqn2 = ion_freq_dist(l,E,self.electron_temperature,m,self.ion_density) - area_e
                    return (eqn1, eqn2)
                def equations_inv(p):
                    m, l = p
                    eqn1 = m - self.ion_density - ((4 * l * np.pi * co.epsilon_0 * rp)/(co.e)) * npp
                ...
                ii = ion_freq_dist(Vpp,E, self.electron_temperature,nee,self.ion_density)
                ed = ((-area_detach)/x) * ne
                ea = ((area_attach)/x) * ne
                print(eqn11, eqn22)
                print('ed/ii:', ed/ii)
                ...
                if np.absolute((Vp - Vpp)/Vp) > tol_iter:
                    ...
                    Te = np.multiply(0.66666666666666666666666, np.trapz(((E ** 1.5)*z1), E))
                    np.savetxt('EEPF_8Pa_dist_photo.txt', np.column_stack([E,z1]), delimiter = ',')
